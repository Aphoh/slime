...
def _start_rollout_servers_external(args) -> tuple[dict[str, RolloutServer], list[Any]]:
    """Bind to an externally managed Dynamo frontend and worker pool."""
    ...
    for worker in workers:
        by_type.setdefault(worker.worker_type, []).append(worker)
    ...
    for worker_type, group_workers in by_type.items():
        ...
        ray.get(group.bind_external_engines(group_workers))
...
def start_rollout_servers(args, pg) -> tuple[dict[str, Any], list[Any]]:
    ...
    if args.rollout_external and getattr(args, "dynamo_frontend_url", None):
        raise ValueError("--rollout-external-engine-addrs and --dynamo-frontend-url are mutually exclusive")
