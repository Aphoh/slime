...
def get_responses(
    ...
    *,
    ...
    """
    assert torch.is_floating_point(logits), f"{logits.dtype}"
...
def _get_local_response_loss_mask(
    ...
    for total_length, response_length, loss_mask in zip(
        total_lengths, response_lengths, loss_masks, strict=False
    ):
        prompt_length = total_length - response_length
...
def policy_loss_function(
    ...
    active_policy_tokens = policy_loss_mask.to(dtype=torch.bool)
    ppo_kl_for_metrics = torch.where(
        active_policy_tokens & torch.isfinite(ppo_kl),
        ppo_kl,
        torch.zeros_like(ppo_kl),
    )

    if args.advantage_estimator == "cispo":
        masked_ppo_kl = torch.where(active_policy_tokens, ppo_kl, torch.zeros_like(ppo_kl))
        masked_log_probs = torch.where(active_policy_tokens, log_probs, torch.zeros_like(log_probs))
        masked_advantages = torch.where(active_policy_tokens, advantages, torch.zeros_like(advantages))
