    ...
    return tuple(
        sorted(
            (
                ...
                info.parallel_config["tp_size"],
                info.parallel_config["pp_size"],
                info.parallel_config["ep_size"],
                info.parallel_config["moe_dp_size"],
                info.server_info.get("dp_size") or info.server_info.get("data_parallel_size"),
            )
            for info in infos
        )
    )
...
class ExternalRolloutServer:
    ...
    should_register_to_router: bool
...
def start_external_rollout_servers(args, *, start_router) -> tuple[dict[str, ExternalRolloutServer], list]:
    ...
    servers = {
        "default": ExternalRolloutServer(
            ...
            args=args,
            engine_infos=infos,
            should_register_to_router=rollout_url is None,
            router_ip=router_ip,
