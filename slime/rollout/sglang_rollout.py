...
async def _generate_dynamo(args: Namespace, sample: Sample, sampling_params: dict[str, Any]) -> Sample:
    ...
    with trace_span(
        sample,
        "dynamo_generate",
        attrs={"max_new_tokens": sampling_params["max_new_tokens"], "prompt_len": len(token_ids)},
    ) as span:
...
async def generate_and_rm(
    ...
    if isinstance(sample, list):
        ...
        for item in samples:
            item.non_generation_time += _rm_elapsed
        return samples
    else:
    ...
    while len(data) < target_data_size:
        ...
        for task in done:
            ...
            _group_total_tokens = sum(
                item.response_length if not isinstance(item, list) else sum(nested.response_length for nested in item)
                for item in group
            )
